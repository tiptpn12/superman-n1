        ...
        <div class="col-md-12">
          ...
          <div class="panel">
            <div class="panel-heading">
              <h3 class="panel-title">Tabel Master Company</h3>
            </div>
                ...
                <tbody>
                  @foreach ($company as $key => $value)
                    <tr>
                      ...
                      <td>{{ $value->domisili_company}}</td>
                      <td>{{ $value->company_status==1 ? "Aktif" : "Tidak Aktif" }}</td>
                      <td>
                        ...
                        <button type="button" class="btn btn-danger btn-sm" onclick="hapus({{ $value->company_id }}, {{ $value->company_status }})" title="Hapus Data" ><i class="fa fa-trash-o" aria-hidden="true"></i> Ubah Status</button>
                      </td>
                    </tr>
                  @endforeach
                </tbody>
...
<div id="modal_tambah" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <form action="{{url('')}}/company/tambah" method="post">
        {{ csrf_field() }}
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modal Tambah Company</h4>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Nama Company</label>
            <input type="text" id="tambah_nama" name="company" class="form-control" placeholder="Nama Company" autocomplete="off" required>
          </div>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Domisili Company</label>
            <input type="text" id="tambah_nama" name="domisili" class="form-control" placeholder="Nama Company" autocomplete="off" required>
          </div>
        </div>       
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <form action="{{url('')}}/company/update" method="post">
        ...
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modal Ubah Data Company</h4>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Nama Company</label>
            <input type="text" id="ubah_nama" name="company" class="form-control" placeholder="Nama Company" autocomplete="off" required>
          </div>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Domisili Company</label>
            <input type="text" id="ubah_domisli" name="domisili" class="form-control" placeholder="Domisili Company" autocomplete="off" required>
          </div>
        </div>
        <div class="modal-footer">
...
<script type="text/javascript">
  ...
  function ubah(data)
  {
    $("#ubah_id").val(data.company_id);
    $("#ubah_nama").val(data.company_nama);
  ...
  function hapus(id, status){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = `{{url('')}}/company/destroy/${id}/${status}`;
      }
    })
  }
</script>
