...
<html>
    <head>
    <title> SPP Khusus | Validasi</title>
	<meta charset="utf-8">
    ...
    <div class="row">
        <div class="col-xs-13">
        @if(isset($sppb) && $sppb !== null)
            <div class="panel" id="panel-sppb" style="background-image: url('{{ asset('assets/img/valid-watermark.jpg')}}');; background-repeat:no-repeat; background-position:center bottom;" >
                <div class="panel-body">
                    ...
                    <div class="row">
                        ...
                        Dengan ini dimohon bantuannya untuk dibuatkan Bukti Keluar Kas / Bank untuk Pembayaran :
                            <table class="table table-bordered wrapped">
                                <thead>
                                <tr>
                                    <th  style="vertical-align:middle;;width:40%">
                                        ...
                                        Nomor Faktur Pajak: {{$sppb[1][0]->faktur_pajak_nomor}}  
                                        <br>
                                        @for($i=1;$i < count($sppb[1]); $i++)
                                        <span class="col-xs-4"></span>{{$sppb[1][$i]->faktur_pajak_nomor}}
                                        <br>
                                        @endfor
                                        Nomor SP/OPL/SPK/Perjanjian : {{$sppb['sppb_sp_opl']}}
                                ...
                                <thead>
                                    <tr>
                                        <th colspan = "4" style="text-align:center;width:40%;"> KODE </th>
                                        <th rowspan = "2" style="width:40%; text-align:center; vertical-align:middle;width:40%" > URAIAN </th>
                                        <th rowspan = "2" style="width:15%; text-align:center; vertical-align:middle;width:20%"> Jumlah <br> Rp.</th>
                                    </tr>
                                    
                                    <tr>
                                        <th style="width:10%;margin:0; text-align:center">KBB</th>
                                        <th style="width:10%;text-align:center">SAP</th>
                                        <th style="width:15%;text-align:center">CC/PC</th>
                                        <th style="width:10%;text-align:center">CF</th>
                                    </tr>
                                </thead>
								...
								@for($i=0;$i< count($sppb[0]); $i++)
                                    <tr>
                                            <td rowspan="{{count($sppb[0][$i][0])}}" style="text-align:center">{{$sppb[0][$i]['master_rekening_kode_kbb']}}</td>
                                            @if($no_vendor_sppb !== null)
                                            <td rowspan="{{count($sppb[0][$i][0])}}" style="text-align:center">
                                                @foreach($no_vendor_sppb as $n)
                                                {{$n}} <br>
                                                @endforeach
                                            </td>
                                            ...
                                            @endif
											<td rowspan="{{count($sppb[0][$i][0])}}" style="text-align:center">{{$sppb[0][$i]['master_cost_center_kode']}} {{$sppb[0][$i]['master_profit_center_kode']}}</td>
                                            <td rowspan="{{count($sppb[0][$i][0])}}" style="text-align:center">{{$sppb[0][$i]['master_cash_flow_kode']}}</td>
                                            <td><?php $a = str_replace(['<p>','</p>'],['','<br />'],$sppb[0][$i][0][0]->sppb_uraian_uraian); ?>
											    {!! $a !!}</td>
                                            <td style="text-align:center"> {{number_format($sppb[0][$i][0][0]->sppb_uraian_nominal)}} </td>
                                            </tr>
                                ...
                                <tr>
                                        ...
                                        <td colspan="4" style="text-transform:uppercase; text-align:center">  <b style="font-style:italic"><?php $m=Terbilang::angka($sppb['sppb_total']); $a=wordwrap($m,50,"<br>",false); ?> {!!$a!!} rupiah </b></td>
            ...
            @if(isset($sppn))
            <div class="panel" id="panel-sppn" style="background-image: url('{{ asset('assets/img/valid-watermark.jpg')}}');; background-repeat:no-repeat; background-position:center bottom;">
                <div class="panel-body">
                    <!-- title row -->
                        ...
                        Dengan ini dimohon bantuannya untuk dibuatkan Bukti Masuk Kas / Bank untuk Penerimaan :
                                ...
                                <thead>
                                   <tr >
                                       ...
                                       BA / AU 58 No : {{$sppn['sppn_ba_au_53']}}
                                        ...
                                        </div>: {{$sppn[1][$i]->faktur_pajak_nomor}}
                                        <br>
                                        @endfor
                            ...
                            <table class="table table-bordered" style="table-layout:fixed;">
                                <thead>    
                                    ...
                                    <tr>
                                        <th style="width:10%;text-align:center">KBB</th>
                                        <th style="width:10%;text-align:center">SAP</th>
                                        <th style="width:15%;text-align:center">CC/PC</th>
                                        <th style="width:10%;text-align:center">CF</th>
                                    </tr>
								...
								@for($i=0;$i < count($sppn[0]); $i++)
                                    <tr>
                                    <td rowspan="{{count($sppn[0][$i][0])}}" style="text-align:center">{{$sppn[0][$i]['master_rekening_kode_kbb']}}</td>
                                    @if($no_vendor_sppn !== null)
                                            ...
                                            @endif
                                            <td rowspan="{{count($sppn[0][$i][0])}}" style="text-align:center">{{$sppn[0][$i]['master_cost_center_kode']}}{{$sppn[0][$i]['master_profit_center_kode']}}</td>
                                            <td rowspan="{{count($sppn[0][$i][0])}}" style="text-align:center">{{$sppn[0][$i]['master_cash_flow_kode']}}</td>
                                            <td>
                                ...
                                <tfoot>
                                <tr>
                                        <td class="terbilang" style="text-align:left"><b>TERBILANG:</b></td>
                                        <td colspan="4" style=" text-align:center"> 
                                            <?php $m = Terbilang::angka($sppn['sppn_jumlah']); $a = wordwrap($m,50,"<br>",false); ?>
                                            <b style="text-transform:uppercase;font-style:italic">  {!! $a !!} rupiah </b></td>
                                        <td style="text-align:center;"> <b>{{number_format($sppn['sppn_jumlah'])}}</td>
                                    </tr>
                                </tfoot>
                            </table>
                            Dokumen-dokumen syarat pembayaran kami lampirkan dan kami bertanggung jawab atas kebenarannya
