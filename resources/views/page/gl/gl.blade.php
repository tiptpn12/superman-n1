...
@section('konten')
    ...
    <div class="main">
        ...
        <div class="main-content">
            <div class="container-fluid">
                <h3 class="page-title">GL</h3>
                <div class="row">
                    <div class="col-md-12">
                        <!-- TABLE -->
                        <div class="panel">
                            <div class="panel-heading">
                                <h3 class="panel-title">Tabel GL</h3>
                            </div>
                    ...
                    <tr>
                      ...
                      <td>{{ $value->master_gl_status == 1 ? "Aktif" : "Tidak Aktif" }}</td>
                      <td>
                        ...
                        <button type="button" class="btn btn-danger btn-sm" onclick="hapus({{ $value->master_gl_id }}, {{ $value->master_gl_status }})" title="Hapus Data" ><i class="fa fa-trash-o" aria-hidden="true"></i> Ubah Status</button>
                      </td>
                    </tr>
                  @endforeach --}}
                                    </tbody>
    ...
    <div id="modal_tambah" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <form action="{{ url('') }}/gl/store" method="post">
                    {{ csrf_field() }}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Modal Tambah Data GL</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Kode GL</label>
                            <input type="text" id="tambah_kode" name="kode" class="form-control" placeholder="Kode GL"
    ...
    <div id="modal_ubah" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
                <form action="{{ url('') }}/gl/update" method="post">
                    {{ csrf_field() }}
                    <input type="hidden" id="ubah_id" name="id">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Modal Ubah Data GL</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Kode GL</label>
                            <input type="text" id="ubah_kode" name="kode" class="form-control" placeholder="GL"
                                autocomplete="off" required>
                        </div>
                        <div class="form-group">
                            <label>Keterangan</label>
    ...
    <div id="modal_import" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
                <form action="{{ route('gl.import') }}" id="form_upload_excel" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title">Upload Data Gl</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group" id="upload_data">
                            <label>Upload data Gl:</label><br>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            tabel = $('#table').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'master_gl_status',
                        name: 'master_gl_status',
                    ...
                    <button type="button" class="btn btn-danger btn-sm" onclick="hapus(${full.master_gl_id}, '${full.master_gl_status}')" title="Hapus Data"><i class="fa fa-trash-o" aria-hidden="true"></i> Ubah Status</button>
                `;
                        }
                    }
                ]
        ...
        function ubah(data) {
            $("#ubah_id").val(data.master_gl_id);
            $("#ubah_kode").val(data.master_gl_kode);
            $("#ubah_keterangan").val(data.master_gl_keterangan);
        ...
        function hapus(id, status) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `{{ url('') }}/gl/destroy/${id}/${status}`;
                }
            })
        }
    </script>
