...
<div class="container-fluid">
    ...
    <div class="panel panel-headline">
        <div class="panel-heading">
            <h3 class="panel-title" id="title_bar_chart_spp">Bar Chart SPP</h3>
        </div>
...
<script>
    ...
    function refreshBarChartSpp(barchart, title, filter_element, data){
        const labels = data.results.map(item => !item.company ? item.master_bagian_kode : item.company)
            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    formatter: (value, context) => {
                        ...
                        return formattedValue.toFixed(2).toLocaleString().replaceAll(',', '.'); // Format the label with commas
                    },
            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    formatter: (value, context) => {
                        ...
                        return formattedValue.toLocaleString().replaceAll(',', '.'); // Format the label with commas
                    },
                    color: 'black', // Color of the label
                    font: {
                        weight: 'light',
    ...
    $(document).ready(() => {
        ...
        .then((responses) => {
                ...
                refreshBarChartSpp(bar_chart_divisi, 'Receipt & Payment Head Office (Dalam Juta)', filter_divisi, responseSppDivisi)
                refreshBarChartSpp(bar_chart_regional, 'Receipt & Payment HO & Reg (Dalam Milliar)', filter_regional, responseSppRegional)
            }
        )
    })

    $(tanggal_awal_divisi).on('change', () => {
        fetchFilteredData(tanggal_awal_divisi, tanggal_akhir_divisi, filter_divisi, bar_chart_divisi, 'Receipt & Payment Head Office (Dalam Juta)', '{{ route('barchart.divisi') }}')
      }
    )
    ...
    $(tanggal_awal_regional).on('change', () => {
        fetchFilteredData(tanggal_awal_regional, tanggal_akhir_regional, filter_regional, bar_chart_regional, 'Receipt & Payment HO & Reg (Dalam Milliar)', '{{ route('barchart.regional') }}')
      }
    )
    $(tanggal_akhir_regional).on('change', () => {
        fetchFilteredData(tanggal_awal_regional, tanggal_akhir_regional, filter_regional, bar_chart_regional, 'Receipt & Payment Head Office (Dalam Juta)', '{{ route('barchart.regional') }}')
      }
    )
</script>
