...
class ChartService
{
    ...
    public function getProses($companyId, $hakAkses)
    {
        if (in_array($companyId, [5, 7, 8, 9, 10, 11, 12, 13, 14])) {
            ...
            // Proses Verifikasi
            $flowDetailByAkses = $this->flowDetailRepository->getDetailFlowByHakAkses(43);
            $todoVerifikasi = $this->prosesRepository->GetProsesToDo($companyId, 43, [1, 2])->count();
            $revisiVerifikasi = $this->prosesRepository->GetProsesRevisi($companyId, 43, $flowDetailByAkses, [3])->count();
            $verifikasi = $todoVerifikasi + $revisiVerifikasi;
    ...
    public function getPenerimaanPengeluaran($companyId, $startMonth, $endMonth)
    {
        ...
        try {
            $totalPembayaran = DB::table('spp')
                ...
                ->where('spp.company_id', $companyId)
