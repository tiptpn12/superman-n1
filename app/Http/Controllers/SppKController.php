...
class SppKController extends Controller
{
    function __construct(){
        $this->middleware(function ($request,$next) {
            ...
            if($this->user == null){
                ...
                return redirect('login')->with('alert','Sesi telah habis! Silahkan Login Terlebih Dahulu');
    ...
    public function upload(Request $request,$id){
        ...
        if($spp->spp_kabag !== null){
            ...
            if($request->upload_file == 'file_baru'){
                ...
                $sppkabags =  $current.'-'.$sppkabag->getClientOriginalName();
        ...
        return redirect('spp_keuangan/send/'.$id);
    }
        ...
        switch($level){
            ...
            case 2:
                ...
                if($status_revisi != null){
                    ...
                    $spp = Spp::find($id);
                    if($spp->spp_jalur_pajak == 1){
                        $spp_proses = SppProses::where('spp_id','=',$id)->first();
                        $spp_proses->spp_proses_petugas_pajak = 1;
                        
                        $spp_proses->save();
                        $sppk = Spp::find($id);
                        $sppk->spp_status_proses++;
                        $sppk->save();
                        $tujuan = 3;
                    }
                    else{
                        $sppk = Spp::find($id);
                        $sppk->spp_status_proses = 7;
                        $sppk->save();
                        $spp_proses = SppProses::where('spp_id','=',$id)->first();
    ...
    public function accept($id){
        ...
        if($spp->spp_jalur_pajak == 0 && $level == 5){
            $a=3;
        }
        if($spp->spp_jalur_pajak == 1 && $level == 5){
            $a=2;
        }
    ...
    ->where('master_rekening_kode_sap', '<>', 0);
        ...
        $b = Bagian::where('master_bagian_id','=',2)->get();
        $sppb_bayar = [];
        $sppn_terima = [];
        ...
        if($level == 1){
            $data=DB::table('spp')->where('spp.master_bagian_id','=',$bagian)->leftJoin('sppb','spp.sppb_id','=','sppb.sppb_id')->leftJoin('sppn','spp.sppn_id','=','sppn.sppn_id')
                ...
                ->select('spp_id','spp.sppb_id','spp.sppn_id','spp_kabag','spp_status_proses','spp_status_posisi','sppb_no',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%d-%m-%Y') as tanggal"),'sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                ->groupBy('spp_id','spp_kabag','spp_status_proses','spp_status_posisi','sppn_tanggal','sppb_no','tanggal','sppb_total','sppn_no','sppn_jumlah','spp_status_ob')
            ...
            foreach($data as $v){
                $sppb_cetak_bukti_kas[] = DB::table('sppb_bukti_kas')->where('sppb_bukti_kas.sppb_id','=',$v->sppb_id)
                                    ->join('master_rekening','sppb_bukti_kas.master_rekening_id','=','master_rekening.master_rekening_id')
                                    ->join('master_vendor','sppb_bukti_kas.master_vendor_id','=','master_vendor.master_vendor_id')
                                    ->select('master_rekening.*','sppb_bukti_kas.*','master_vendor.*')->first();
                $sppn_cetak_bukti_kas[] = DB::table('sppn_bukti_kas')->where('sppn_bukti_kas.sppn_id','=',$v->sppn_id)
                                    ->join('master_rekening','sppn_bukti_kas.master_rekening_id','=','master_rekening.master_rekening_id')
                                    ->join('master_vendor','sppn_bukti_kas.master_vendor_id','=','master_vendor.master_vendor_id')
                                    ->select('master_rekening.*','sppn_bukti_kas.*','master_vendor.*')->first();
            }
        }
            ...
            if(empty($rekam_jejak_selesai)){
                ...
                //$data = collect($data)->sortByDesc('spp_tanggal');
               ...
               $index_cetak =Session::get('index_cetak');
                ...
                // dd($posisi);
         // dd($sppb_bayar,$sppn_terima);
        return view ('page.spp_keuangan.spp_keuangan', compact ('index_cetak','id_cetak','vendor','index','data','data_batal','rekam_jejak_batal','posisi','status','status_revisi','rekam_jejak','sppb_cetak_bukti_kas','sppn_cetak_bukti_kas','sppb_bayar','sppn_terima','rekening','b','bagian_id','posisi_batal','data_selesai','rekam_jejak_selesai','sppb_bayar_selesai','sppn_terima_selesai','asal'));
    ...
    public function update(Request $request, $id){
       ...
       // dd($request->all());
        ...
        else if(isset($idsppn) && isset($idsppb)){
            $form=3;
        }
        ...
        if($form == 1){
            ...
            $sppb -> master_bagian_id = 2;
            $sppb -> sppb_no = $nomor;
            $sppb -> sppb_jenis = 'Keuangan';
            $sppb -> sppb_urutan = $request -> urutan_sppb;
            ...
            foreach($idisisppb as $i => $v){
                $isi=IsiSppb::findOrFail($v->sppb_isi_id);
                $isi->delete();
                foreach($iduraiansppb[$i] as $u => $va){
                    $uraian = IsiUraianSppb::findOrFail($va->sppb_uraian_id);
                    $uraian->delete();
                }
            }
            $isisppb=$request->isi_sppb;
            ...
            $dokpenlamasppb = $request->dokpenlama_sppb;
            //dd($dokpenlama);
            foreach($dokpensppbtersimpan as $i => $v){
                if(empty($dokpenlamasppb[$i])){
                    $dok=DokumenPendukungSppb::findorFail($v->dokumen_pendukung_sppb_id);
                    // dd($dok->dokumen_pendukung_sppb_nama);
            ...
            foreach($dokpensppntersimpan as $i => $v){
                if(empty($dokpenlamasppn[$i])){
                    $dok=DokumenPendukungSppn::findorFail($v->dokumen_pendukung_sppn_id);
            ...
            if($dokpensppn != null)
                ...
                foreach($idisisppn as $i => $v){
                    $isi=IsiSppn::findOrFail($v->sppn_isi_id);
                    $isi->delete();
                    foreach($iduraiansppn[$i] as $u => $va){
                        $uraian = IsiUraianSppn::findOrFail($va->sppn_uraian_id);
                        $uraian->delete();
                    }
                }
                    ...
                    foreach( $isisppn as $isi =>$value){
                        if($value['jenis_center']=='cost_center'){
                            ...
                            if($value['jenis_sap'] == 'vendor'){
                                ...
                                $isisppn->master_customer_id = null;
                                $isisppn->master_gl_id = null;
    
                            }elseif($value['jenis_sap'] == 'customer'){
                                $isisppn->master_kode_vendor_id = null;
                                $isisppn->master_gl_id = null;
            ...
            $request->request->add(['sppb_id'=>$sppb->sppb_id]);
            foreach($faktur_pajak_sppb as $key => $value){
                $fp_sppb = FakturPajak::find($value->faktur_pajak_id);
                $fp_sppb->delete();
            }
            ...
            if($dokumenpendukungsppn != null)
                ...
                foreach( $isisppn as $isi =>$value){
                    ...
                    else if ($value['jenis_center']=='profit_center'){
                        $isisppn = new IsiSppn;
                        $isisppn->sppn_id=$request->sppn_id;
                        $isisppn->master_kode_kbb = $value['kode_kbb'];
    ...
    public function viewdetail($id){
        ...
        if($idsppb != null){
            ...
            ->select('sppb_isi.*','master_customer.*','master_gl.*','master_rekening.*','master_cost_center.*','master_cash_flow.*')->get();
        ...
        if($idsppn != null){
            ...
            ->select('sppn_isi.*','master_customer.*','sppn_isi.*','master_gl.*','master_rekening.*','master_cost_center.*','master_profit_center.*','master_cash_flow.*')->get();
    ...
    ->where('master_rekening_kode_sap', '<>', 0);
        ...
        $idspp = DB::table('spp')->where('spp.spp_id','=',$id)->select('sppb_id','sppn_id','spp_jalur_pajak')->first();
        $idsppb = $idspp->sppb_id;
        $idsppn = $idspp->sppn_id;
        $bagian = 2;
        $gl = GL::All();
        ...
        if($idsppb != null){
            ...
            ->leftjoin('master_cash_flow','sppb_isi.master_cash_flow_id','=','master_cash_flow.master_cash_flow_id')
            ->select('sppb_isi.*','master_customer.*','master_gl.*','master_rekening.*','master_profit_center.*','master_cost_center.*','master_cash_flow.*')->get();
        ...
        if($idsppn != null){
            ...
            ->leftJoin('master_profit_center','sppn_isi.master_profit_center_id','=','master_profit_center.master_profit_center_id')
            ->leftjoin('master_cash_flow','sppn_isi.master_cash_flow_id','=','master_cash_flow.master_cash_flow_id')
            ->select('sppn_isi.*','master_customer.*','master_rekening.*','master_gl.*','master_cost_center.*','master_profit_center.*','master_cash_flow.*')->get();
        ...
        if(isset($data_sppb) && empty($data_sppn)){
            $form=1;
                $Krywn_sppb = DB::table('nama_karyawan')->where('nama_karyawan.sppb_id','=',$data_sppb['sppb_id'])->select('nama_karyawan.*')->get();
                foreach($Krywn_sppb as $k => $v){
                    $karyawan_nama_sppb[] = $v->karyawan_nama;
                }
                $karyawan_nama_sppn = null;
                $Krywn_sppn =null;

        }
        else if(isset($data_sppn) && empty($data_sppb)){
            $form=2;
            $Krywn_sppn = DB::table('nama_karyawan')->where('nama_karyawan.sppn_id','=',$data_sppn['sppn_id'])->select('nama_karyawan.*')->get();
                foreach($Krywn_sppn as $k => $v){
                    $karyawan_nama_sppn[] = $v->karyawan_nama;
                }
                $karyawan_nama_sppb = null;
                $Krywn_sppb =null;
        }
        else{
            $form=3;
        ...
        $data = array(
            ...
            'vendor' => $vendor,
            'karyawan_sppb' => $Krywn_sppb,
            'karyawan_sppn' => $Krywn_sppn,
            'gl' => $gl
            
        );
        
            return view ('page.spp_keuangan.spp_keuangan_edit',$data);
    }
    ...
    public function cetak($id){
        ...
        if($idsppb != null){
            ...
            ->leftjoin('master_cash_flow','sppb_isi.master_cash_flow_id','=','master_cash_flow.master_cash_flow_id')
            ->select('sppb_isi.*','master_customer.*', 'master_gl.*','master_rekening.*','master_cost_center.*','master_profit_center.*','master_cash_flow.*')->get();
            $faktur_pajak_sppb = DB::table('faktur_pajak')->where('faktur_pajak.sppb_id','=',$datasppb->sppb_id)->select('faktur_pajak_nomor')->get();
            foreach($sppbisi as $a =>$value2){
                 $sppburaian[] = DB::table('sppb_uraian')->where('sppb_uraian.sppb_isi_id','=',$value2->sppb_isi_id)->select('sppb_uraian.*')->get();
             }
        ...
        if($idsppn != null){
            ...
            ->leftJoin('master_profit_center','sppn_isi.master_profit_center_id','=','master_profit_center.master_profit_center_id')
            ->leftjoin('master_cash_flow','sppn_isi.master_cash_flow_id','=','master_cash_flow.master_cash_flow_id')
            ->select('sppn_isi.*','master_customer.*', 'master_gl.*','master_rekening.*','master_cost_center.*','master_profit_center.*','master_cash_flow.*')->get();
    ...
    public function bayar(Request $request,$id){
        ...
        // dd($request->tanggal_bayar_sppb);
            ...
            $bukti =  $current.'-'.$file_bukti->getClientOriginalName();
            $file_bukti->move('dokumen/',$bukti);
                $tanggal1 = $request->tanggal_terima_sppn;
            $tanggals1 = date('Y-m-d',strtotime($tanggal1));
            $terima = new SppnTerima;
            $terima->sppn_id = $id;
            $terima->sppn_terima_nomor_bukti_kas = $request->nomor_bukti_kas_sppn;
            $terima->sppn_terima_tanggal = $tanggals1;
            $terima->master_rekening_id = $request->rekening_bank_sppn;
            $terima->sppn_terima_bukti = $bukti;
            $terima->save();
    ...
    public function cetakbuktikas($id){
        ...
        if($idsppb != null){
            ...
            ->join('master_vendor','sppb_bayar.master_vendor_id','=','master_vendor.master_vendor_id')
...
// dd($databayar);
            ...
            ->join('master_rekening','sppb_bukti_kas.master_rekening_id','=','master_rekening.master_rekening_id')
            ->join('master_vendor','sppb_bukti_kas.master_vendor_id','=','master_vendor.master_vendor_id')
            ->select('sppb_bukti_kas.*','master_rekening.*','master_vendor.*')->first();
            $sppbisi = DB::table('sppb_isi')->where('sppb_isi.sppb_id','=',$datasppb->sppb_id)
            ->leftJoin('master_rekening','sppb_isi.master_kode_vendor_id','=','master_rekening.master_rekening_id')
            ->leftJoin('master_gl','sppb_isi.master_gl_id','=','master_gl.master_gl_id')
            ->leftJoin('master_cost_center','sppb_isi.master_cost_center_id','=','master_cost_center.master_cost_center_id')
            ...
            ->leftJoin('master_cash_flow','sppb_isi.master_cash_flow_id','=','master_cash_flow.master_cash_flow_id')
            ->select('sppb_isi.*','master_rekening.*','master_cost_center.*','master_profit_center.*','master_cash_flow.*','master_gl.*')->get();
        ...
        if($idsppn != null){
            ...
            $dataterima_vendor = DB::table('sppn_terima')->where('sppn_id','=',$idsppn)
            ->join('master_vendor','sppn_terima.master_vendor_id','=','master_vendor.master_vendor_id')
            ->select('sppn_terima.*','master_vendor.*')->first();
            $databuktikassppn = DB::table('sppn_bukti_kas')->where('sppn_id','=',$idsppn)
            ->join('master_rekening','sppn_bukti_kas.master_rekening_id','=','master_rekening.master_rekening_id')
            ->join('master_vendor','sppn_bukti_kas.master_vendor_id','=','master_vendor.master_vendor_id')
            ->select('sppn_bukti_kas.*','master_rekening.*','master_vendor.*')->first();
        ...
        if (isset($data_sppb['sppb_tanggal']) && empty($data_sppn['sppn_tanggal'])) {
        ...
        }else if (isset($data_sppn['sppn_tanggal'])  && empty($data_sppb['sppb_tanggal'])) {
        ...
        // dd($databuktikassppb,$databuktikassppn);
        $data = array(
    ...
    ->where('master_rekening_kode_sap', '<>', 0);
        ...
        if($level == 1){
            ...
            if($vendor_filter !== "semua"){
                $data[] = DB::table('spp')->where('spp.master_bagian_id','=',2)->leftJoin('sppb','spp.sppb_id','=','sppb.sppb_id')->leftJoin('sppn','spp.sppn_id','=','sppn.sppn_id')
                        ...
                        ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                        ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                        ->select('spp_id','spp.sppb_id','spp.sppn_id','sppb.master_bank_id','sppn.master_bank_id','spp.master_bagian_id','master_bagian_nama','spp_status_proses','spp_status_posisi','spp_status_lunas',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%Y-%m-%d') as tanggal"),'sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                        ->where('sppb.master_bank_id','=',$vendor_filter)->orWhere('sppn.master_bank_id','=',$vendor_filter)
                        ->groupBy('spp_id','spp_status_proses','spp_status_posisi','spp_status_lunas','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                        ->get();
            }
            else{
                $data[] = DB::table('spp')->where('spp.master_bagian_id','=',2)->leftJoin('sppb','spp.sppb_id','=','sppb.sppb_id')->leftJoin('sppn','spp.sppn_id','=','sppn.sppn_id')
                        ...
                        ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                        ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                        ->select('spp_id','spp.sppb_id','spp.sppn_id','spp.master_bagian_id','master_bagian_nama','spp_status_proses','spp_status_posisi','spp_status_lunas',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%Y-%m-%d') as tanggal"),'sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                        ->groupBy('spp_id','spp_status_proses','spp_status_posisi','spp_status_lunas','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                        ->get();
            }
                    $data = $data[0];
            ...
            foreach($spp as $s){
                if($s->master_bagian_id == 2){
                    ...
                    if($vendor_filter !== "semua"){
                        ...
                        ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                        ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                        ->select('spp_id','sppb.master_bank_id','sppn.master_bank_id','spp.master_bagian_id','master_bagian_nama','spp_status_proses','spp_status_posisi',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%Y-%m-%d') as tanggal"),'sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                        ->where('sppb.master_bank_id','=',$vendor_filter)->orWhere('sppn.master_bank_id','=',$vendor_filter)
                        ->groupBy('spp_id','spp_status_proses','spp_status_posisi','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                        ->first();
                    }else{
                        ...
                        ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                        ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                        ->select('spp_id','spp.master_bagian_id','master_bagian_nama','spp_status_proses','spp_status_posisi',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%Y-%m-%d') as tanggal"),'sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                        ->groupBy('spp_id','spp_status_proses','spp_status_posisi','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                        ->first();
                    }
            ...
            foreach($spp as $s){
                if($s->master_bagian_id == 2){
                    ...
                    if($vendor_filter !== "semua"){
                        ...
                        ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                        ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                        ->select('spp_id','sppb.master_bank_id','sppn.master_bank_id','spp.master_bagian_id','master_bagian_nama','spp.sppb_id','spp_status_posisi','spp_status_bayar','spp_status_terima','spp.sppn_id','spp_status_proses','spp_status_bayar',DB::raw("DATE_FORMAT(spp.spp_tanggal,'%Y-%m-%d') as tanggal"),'sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                        ->where('sppb.master_bank_id','=',$vendor_filter)->orWhere('sppn.master_bank_id','=',$vendor_filter)
                        ->groupBy('spp_id','spp.sppb_id','spp_status_posisi','spp_status_bayar','spp_status_terima','spp.sppn_id','spp_status_proses','spp_status_bayar','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                        ->first();
                    }else{
            ...
            if($vendor_filter !== "semua"){
                $data[]=DB::table('spp')->where('spp.master_bagian_id','=',$bagian)->leftJoin('sppb','spp.sppb_id','=','sppb.sppb_id')->leftJoin('sppn','spp.sppn_id','=','sppn.sppn_id')
                    ...
                    ->leftJoin('sppn_isi','sppn.sppn_id','=','sppn_isi.sppn_id')->leftJoin('sppn_uraian','sppn_isi.sppn_isi_id','=','sppn_uraian.sppn_isi_id')
                    ->leftJoin('master_bagian','spp.master_bagian_id','=','master_bagian.master_bagian_id')
                    ->select('spp_id','sppb.master_bank_id','sppn.master_bank_id','spp.master_bagian_id','master_bagian_nama','spp_status_proses','spp_status_posisi','spp_status_bayar','spp.created_at as tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob',DB::raw("GROUP_CONCAT(DISTINCT sppb_uraian_uraian SEPARATOR ',') as sppb_uraian2"),DB::raw("GROUP_CONCAT(DISTINCT sppn_uraian_uraian SEPARATOR ',') as sppn_uraian2"))
                    ->where('sppb.master_bank_id','=',$vendor_filter)->orWhere('sppn.master_bank_id','=',$vendor_filter)
                    ->groupBy('spp_id','spp_status_proses','spp_status_posisi','spp_status_bayar','tanggal','sppb_no','sppb_tanggal','sppb_total','sppn_no','sppn_tanggal','sppn_jumlah','spp_status_ob')
                    ->get();
            }else{
        ...
        elseif($request->index_advanced_search == "2"){
            ...
            if($request->vendor !== 'semua'){
                $data_batal = $data_batal->where('master_bank_id',$request->vendor);
            }
        ...
        if(isset($data)){
            ...
            if(empty($rekam_jejak_batal)){
                ...
                //dd($rentang_waktu,$data,$data_selesai);
        return view ('page.spp_keuangan.spp_keuangan', compact ('vendor','index_cetak','id_cetak','index','data','data_selesai','data_batal','rekam_jejak_selesai','rekam_jejak_batal','posisi','status','status_revisi','rekam_jejak','sppb_bayar','sppn_terima','rekening','b','bagian_id','posisi_batal','asal'));
