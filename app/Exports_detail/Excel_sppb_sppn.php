...
class Excel_sppb_sppn implements FromView, ShouldAutoSize, WithTitle
{
    protected $rentang_waktu, $status_bayar, $jenis_spp, $company_id, $title, $bagian, $hakakses;
    ...
    public function view(): View
    {
        ...
        if ($this->rentang_waktu !== "semua") {
            ...
            if ($this->jenis_spp == "semua") {
                ...
                $sppb_sppn = DB::table('spp_proses')
                    ->where(
                        'spp_proses.spp_proses_petugas_kas_dan_bank',
                        '=',
                        NULL
                    )->join('spp', 'spp_proses.spp_id', '=', 'spp.spp_id')
                    ->where('spp.spp_status_ob', '!=', 2)
        ...
        // ambil data berdasarkan status bayar
        if ($this->status_bayar !== "semua") {
            ...
            foreach ($spp as $s) {
                $datanya[] = DB::table('spp')->where('spp.spp_id', '=', $s->spp_id)
                    ...
                    ->leftJoin('master_vendor as vendor_isi', 'sppb_isi.master_kode_vendor_id', '=', 'vendor_isi.master_vendor_id')
                    ->leftJoin('master_vendor as vendor_bank', 'vendor_bank.master_vendor_id', '=', 'sppb.master_bank_id')
                    ->select(
                        'spp_id',
                        'sppd_posisi',
                        'master_cost_center.*',
                        ...
                        'sppb_total',
                        'spp_status_ob',
                        'sppb_uraian_uraian  as sppb_uraian2',
                        'sppb_bayar.sppb_id as cek_bukti',
                        ...
                        'sppb.master_bank_id',
                        'vendor_bank.master_vendor_rekening',
                        'vendor_bank.master_vendor_nama'
                    )->where('spp.company_id', '=', $this->company_id)
                    ->where('spp.spp_status_bayar', '=', $this->status_bayar)->get();
                // dd($datanya);
                $datanya[] = DB::table('spp')->where('spp.spp_id', '=', $s->spp_id)
        ...
        foreach ($datanya as $d1) {
            foreach ($d1 as $d) {
                if ($d->spp_id) {
                    $data[] = $d;
                    if ((property_exists($d, 'sppn_terima_nomor_bukti_kas') && $d->sppn_terima_nomor_bukti_kas == null) || (property_exists($d, 'sppb_bayar_nomor_bukti_kas') && $d->sppb_bayar_nomor_bukti_kas == null)) {
                        $cek = 0;
                    }
                }
            }
        ...
        // dd($data);
        return view('page.laporan.laporan_excel_detail', compact('sum_spp', 'data', 'sppbisi', 'sppnisi', 'sppb_bayar', 'sppn_terima', 'rentang_waktu', 'karyawan_no_vendor_sppb', 'karyawan_no_vendor_sppn', 'posisi_dinamis'));
    }
